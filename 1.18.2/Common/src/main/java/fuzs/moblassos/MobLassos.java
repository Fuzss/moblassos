...
public class MobLassos implements ModConstructor {
    ...
    private static void registerHandlers() {
        PlayerInteractEvents.USE_ENTITY_AT.register(LassoItem::onEntityInteract);
        PlayerInteractEvents.USE_ENTITY_AT.register(ContractItem::onEntityInteract);
        ServerEntityLevelEvents.LOAD.register(ContractItem::onEntityJoinServerLevel);
    }

    @Override
    public void onRegisterCreativeModeTabs(CreativeModeTabContext context) {
        context.registerCreativeModeTab(CreativeModeTabConfigurator.from(MOD_ID).icon(() -> new ItemStack(ModRegistry.GOLDEN_LASSO_ITEM.get())).displayItems((output) -> {
