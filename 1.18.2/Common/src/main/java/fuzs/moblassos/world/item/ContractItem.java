    ...
    public static EventResultHolder<InteractionResult> onEntityInteract(Player player, Level level, InteractionHand hand, Entity entity, Vec3 hitVector) {
        ...
        if (stack.getItem() instanceof ContractItem && entity instanceof AbstractVillager abstractVillager && entity.isAlive()) {
            ...
            if (abstractVillager instanceof Villager villager) {
                ...
                Component merchantLevel = new TranslatableComponent("merchant.level." + villager.getVillagerData().getLevel());
                displayName = new TextComponent("").append(entity.getDisplayName()).append(" (").append(merchantLevel).append(")");
            } else {
            ...
            if (VillagerContractCapability.canAcceptContract(abstractVillager)) {
                return ModRegistry.VILLAGER_CONTRACT_CAPABILITY.maybeGet(entity).filter(Predicate.not(VillagerContractCapability::hasAcceptedContract)).map(capability -> {
                    ...
                    player.displayClientMessage(new TranslatableComponent(ModRegistry.CONTRACT_ITEM.get().getDescriptionId() + ".accept", displayName).withStyle(ChatFormatting.GREEN), true);
                ...
                player.displayClientMessage(new TranslatableComponent(ModRegistry.CONTRACT_ITEM.get().getDescriptionId() + ".reject", displayName).withStyle(ChatFormatting.RED), true);
